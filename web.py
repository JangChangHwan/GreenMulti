...
#		try:
			...
			if not kbuid: kbuid = self.u_InputBox(u'넓은마을 로그인', u'아이디')
			kbupw = self.Decrypt(self.ReadReg('kbupw'))
			if not kbupw: kbupw = self.u_InputBox(u'넓은마을 로그인', u'비밀번호', pwd=1) 
			if not kbuid or not kbupw: return self.u_msgbox(u'알림', u'사용자 아이디와 비밀번호는 필수 입력사항입니다.')
			...
			if "login=true" in self.soup.get_text():
				...
				self.WriteReg('kbuid', '')
				self.WriteReg('kbupw', '')
				return self.u_MsgBox(u'알림', u'넓은마을 로그인에 실패했습니다.')
